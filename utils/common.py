...
def get_author_phone_numbers():
    ...
    author_phone_numbers = OdoqModels.User.objects.filter(Q(grade=1) | Q(grade=2)).filter(accept_sms=True).values_list(
        'phone', flat=True)
...
def get_student_phone_numbers():
    ...
    student_phone_numbers = OdoqModels.User.objects.filter(grade=0).filter(accept_sms=True).values_list('phone',
                                                                                                        flat=True)
